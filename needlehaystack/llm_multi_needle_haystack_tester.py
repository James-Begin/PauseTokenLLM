    ...
    async def insert_needles(self, context, depth_percent, context_length):
        ...
        if self.provider == "huggingface":
            total_needles_length = sum(
                len(self.model_to_test.encode_text_to_tokens(needle, no_bos=True)) for needle in self.needles)
        else:
        ...
        for i in range(len(self.needles)):
            ...
            if depth_percent == 100:
                ...
                print(
                    f"Inserted '{self.needles[i]}' at {insertion_percentage:.2f}% of the context, total length now: {len(tokens_context)} tokens")
